...
Presteps before use it:
    ...
    1) Redifine you file_path  in the function pushLoadImage1 and pushLoadImage2.
    So that when you load your images the right folder place is opened.
    
    2) Ensure to have all the python libraries needed. Otherwise you need to install them.
    For example scipy is not available directly in all the versions.
    
    3) Change the molecular library with your coefficients and molecules (line 295 to 305).
    If you are not familiar with trees construction in python, you can enter new coefficients directly in the GUI.
    
    4) If you are facing problems to load images try to use the more adapted format: .png
    
If you do a wrong operation a message will pop up and gide you in the GUI. Enjoy!
...
#                                                            ("BMP Files", "*.bmp")))
    ...
    def plotsmoothed(self):
        ...
        if (self.chksmooth.get()):
            ...
            print(Kernel_size)                                                  # accessing the slider value
    ...
    def Plot(self):
        ...
        if self.I1_ar is None:
            ...
            messagebox.showinfo ("warning","Before, select a molecule in the library")
            
        else:
            if (self.chkcalibration.get()):                                         # If user wants to apply calibration
                if (self.chknormalization.get()):                                   # calibration and noralization
                    ...
                    max_value=np.max(I1_ar_con_norm_cal)
                    print(max_value)
                    print(I1_ar_con_norm_cal)
                    # Make a surface plot
                    fig, ax = plt.subplots(subplot_kw={"projection": "3d"})
